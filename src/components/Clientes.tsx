...
const logosFila1 = [
  ...
  { src: logoDolfin, nombre: "Dolphin Discovery" },
  { src: logoBarcelo, nombre: "Barceló Hotels" },
  { src: logoCadu, nombre: "CADU Inmobiliaria" },
  { src: logoCFE, nombre: "Comisión Federal de Electricidad (CFE)" },
  { src: logoCiudad, nombre: "Ciudad Mayakoba" },
  { src: logoCumbres, nombre: "Residencial Cumbres Cancún" },
];

const logosFila2 = [
  { src: logoLomas, nombre: "Lomas Travel" },
  { src: logoWalmart, nombre: "Walmart México" },
  { src: logoWestin, nombre: "Westin Hotels & Resorts" },
  ...
  { src: logoXcaret, nombre: "Xcaret México" },
...
const Clientes: React.FC = () => {
  return (
    <section className="clientes" id="clientes">
      <div className="clientes-container" aria-label="Empresas que confían en Grupo SAOM para su seguridad">
        <h2 className="clientes-title">Empresas que Confían en Grupo SAOM</h2>
        <p className="clientes-subtitle">
          Más de 15 empresas líderes en Cancún y la Riviera Maya utilizan nuestras cámaras de videovigilancia y tecnología de seguridad.
        </p>
          ...
          <div className="clientes-slide-track">
            {[...logosFila1, ...logosFila1].map((logo, index) => (
              <div key={`fila1-${index}`} className="clientes-slide">
                <img
                  src={logo.src.src}
                  alt={`Cliente destacado: ${logo.nombre}`}
                  loading="lazy"
                  aria-hidden={index >= logosFila1.length ? "true" : "false"}
            ...
            {[...logosFila2, ...logosFila2].map((logo, index) => (
              <div key={`fila2-${index}`} className="clientes-slide">
                <img
                  ...
                  alt={`Cliente destacado: ${logo.nombre}`}
                  loading="lazy"
                  aria-hidden={index >= logosFila2.length ? "true" : "false"}
                  className="cliente-logo"
                />
              </div>
            ))}
